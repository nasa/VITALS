...
# Modified from 
def calc_ewt(filepath, outdir, n_cpu=(cpu_count()-1)) -> None:
    """
    This function will calculate the equivalent water thickness (EWT) or canopy water content (CWC) from an EMIT .nc reflectance file using `ray` for parallelization.

    """
    # Get number of rows to break up in parallel
    emit_ds = xr.open_dataset(filepath)
    ...
    if 'Orthorectified' in emit_ds.attrs.keys():
        ortho_cwc = True
    else:
        ortho_cwc = False
    ...
    output_metadata = {}
    keep = ['flight_line', 'time_coverage_start', 'time_coverage_end','easternmost_longitude','northernmost_latitude','westernmost_longitude',
    'southernmost_latitude','spatialResolution','spatial_ref','geotransform','day_night_flag','title','granule_id']
    for key in keep:
        output_metadata[key]=emit_ds.attrs[key]
    output_metadata['title'] = 'EMIT Estimated Equivalent Water Thickness (EWT) / Canopy Water Content (CWC)'

    # Create Data Vars - Multiply by 10 to get cm and squeeze bands dim
    data_vars = {'cwc':(list(emit_ds.dims.keys())[0:2], 10*cwc.squeeze())}

    ds_cwc = xr.Dataset(data_vars=data_vars, coords=coords, attrs=output_metadata)
    ds_cwc.cwc.attrs = {'long_name':'Canopy Water Content','units':'g/cm^2','_FillValue':-9999}
    ...
    # Create output cog
    ds_cwc.rio.to_raster(raster_path=f"{outdir}{ds_cwc.attrs['granule_id']}_cwc.tif", driver="COG")
...
def run_lines(
    ...
    rfl = np.nan_to_num(xr.open_dataset(rfl_file)['reflectance'][start_line:stop_line,:,:].data,nan=-9999)
    rs, cs, zs = rfl.shape
    ...
    for r in range(rfl.shape[0]):
        for c in range(rfl.shape[1]):
            ...
            if np.all(meas < 0):
                continue
            output_cwc[r, c, 0] = invert_liquid_water(meas, wl)[0]
